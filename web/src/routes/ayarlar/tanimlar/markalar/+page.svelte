<script lang="ts">
  ...
  import type { ProductBrand } from '$lib/features/products/types';

  let session = $state<Session>();
  let items = $state<ProductBrand[]>([]);
  let name = $state('');
  ...
  async function save(event: SubmitEvent) {
    ...
    error = '';
    try {
      const item = await createProductBrand(name);
  ...
  async function toggle(item: ProductBrand) {
...
<svelte:head><title>Stok Markaları · Varya One</title></svelte:head>

<header class="page-header">
  <div>
    <h1>Stok markaları</h1>
    <p>Stok kartlarında kullanılabilecek marka tanımlarını yönetin.</p>
  </div>
</header>
...
{#if loading}
  <div class="card">Markalar yükleniyor…</div>
{:else}
  <div class="workspace-grid">
    <form class="card form" onsubmit={save}>
      <h2 class="panel-title">Yeni marka</h2>
      <label class="field"
        >Tanım adı<input bind:value={name} required maxlength="120" disabled={!canManage} /></label
      >
      <div class="form-actions">
        <button class="button" disabled={!canManage || saving}>Marka ekle</button>
      </div>
    </form>
    <section class="card">
      <h2 class="panel-title">Tanımlı markalar</h2>
      {#if items.length === 0}
        <p class="hint">Henüz marka tanımlanmadı.</p>
      {:else}
        <div class="stack">
          {#each items as item}
