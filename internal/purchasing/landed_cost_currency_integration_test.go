...
// always was.
func TestForeignLandedCostUsesBaseCurrencyRate(t *testing.T) {
	...
	service := NewService(pool, nil, nil, landedCostTestRate{})
	created, err := service.CreateLandedCost(ctx, session, LandedCostInput{
		GoodsReceiptID: receiptID, Amount: "123.45", Currency: "USD", AllocationMethod: LandedCostByAmount,
		Description: "Navlun",
	...
	if err = pool.QueryRow(ctx, `SELECT sum(base_allocated_amount)::text FROM purchase_landed_cost_lines WHERE company_id=$1 AND landed_cost_id=$2`, companyID, created.ID).Scan(&draftBase); err != nil || compare(draftBase, "4938") != 0 {
		t.Fatalf("draft base=%s: %v", draftBase, err)
	}
	...
	if err = pool.QueryRow(ctx, `SELECT sum(base_amount)::text FROM stock_cost_adjustments WHERE company_id=$1 AND source_id=$2`, companyID, posted.ID).Scan(&baseTotal); err != nil {
		t.Fatal(err)
	}
	t.Logf("123.45 USD at 40 TRY/USD: stored base cost=%s TRY", baseTotal)
	if compare(baseTotal, "4938") != 0 {
		t.Errorf("foreign landed cost should multiply the rate: expected 4938 TRY")
	}
}

type landedCostTestRate struct{}

func (landedCostTestRate) ResolveRate(context.Context, string, string, time.Time) (string, error) {
	return "40", nil
}
